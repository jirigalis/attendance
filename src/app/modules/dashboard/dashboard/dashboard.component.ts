...
export class DashboardComponent implements OnInit {
	...
	private _getMembersAgeData(data) {
		...
		data.forEach(val => {
			if (val.role == "D") {
				...
				let index = memberAgeData.findIndex(find => MemberService.getAgeFromRC(val.rc) == find.name);
				if (index < 0) {
    ...
    private _getMembersCountData(data) {
        let membersCountData = [
			{
				name: 'Dívky',
				value: [data.filter(val => MemberService.isWomanFromRC(val.rc) && val.role != "V").length],
